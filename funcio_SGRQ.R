# Funció per calcular Sant GEORGE una base de dades amb els items corresponens###
...
dades_TAI<-dades_TAI %>% mutate_at(vars(starts_with("SG")), ~if_else(is.na(.),9,.)) 


#  S'ha de recodificar les variables amb mútiples items del 2 al 0
...
# En la variable SG10 hi ha valors no v�lids

# IF (SG10 EQ 4) SG10=9.

dades_TAI %>% mutate(SG10=if_else(SG10==4,9,SG4)) %>% select(SG4)
