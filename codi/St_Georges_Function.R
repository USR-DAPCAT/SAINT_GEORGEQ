...
if(count<50)
  ...
  return(print("Hi falta alguna variable!!!!!!!!!. no podem fer  la funció : SANT_GEORGES "))
...
# 0_iii)
#Tot el que sigui difernet a l'interval de cada qüestió serà convertit en missing!
#---------------------------------------------------------------------------#
#sg1      --->    .1.2.3.4.5.9
#sg2      --->    .1.2.3.4.5.9
#sg3      --->    .1.2.3.4.5.9
...
A <- compareGroups( ~ SYMPTOM + ACTIVIT+IMPACTS+TOTALSG, data = dt) 
B <- compareGroups( ~ SYMPTOM + ACTIVIT+IMPACTS+TOTALSG, data = dt,
                        method = c(SYMPTOM = 2,ACTIVIT=2,IMPACTS=2,TOTALSG=2 )) 
#--------------------------------------------------------------------------------------------#
result1<-createTable(A)
result2<-createTable(B)
result3<-summary(dt)
CORRELATIONS<-cor(dt, method = c("pearson", "kendall", "spearman"))
#--------------------------------------------------------------------------------------------#
OUTPUT <- list(dt,result1,result2,result3,CORRELATIONS)
